...
void DACC::set_dac_chA(uint16_t _out) {
  ...
  LL_DAC_ConvertData12RightAligned(DAC, LL_DAC_CHANNEL_1, _out);
  LL_DAC_TrigSWConversion(DAC, LL_DAC_CHANNEL_1);
}

void DACC::set_dac_chB(uint16_t _out) {
  _out = (_out > DAC_OUT_MAX) ? DAC_OUT_MAX : _out;
  LL_DAC_ConvertData12RightAligned(DAC, LL_DAC_CHANNEL_2, _out);
...
void DACC::set_dacs(uint16_t _out_a, uint16_t _out_b) {
  ...
  LL_DAC_ConvertData12RightAligned(DAC, LL_DAC_CHANNEL_1, _out_a);
  LL_DAC_ConvertData12RightAligned(DAC, LL_DAC_CHANNEL_2, _out_b);
  LL_DAC_TrigSWConversion(DAC, LL_DAC_CHANNEL_1);
  LL_DAC_TrigSWConversion(DAC, LL_DAC_CHANNEL_2);
}

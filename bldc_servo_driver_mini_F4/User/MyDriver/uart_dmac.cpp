...
void UART_DMAC::init_rx() {
  ...
  LL_DMA_ConfigAddresses((DMA_TypeDef *)dma_, (uint32_t)dma_ch_rx_,
                         LL_USART_DMA_GetRegAddr((USART_TypeDef *)usart_),
...
uint16_t UART_DMAC::get_rxBuf_head() {
  DMA_Stream_TypeDef *_dma_ch = (DMA_Stream_TypeDef *)((uint32_t)dma_ + 0x10 + 0x18 * dma_ch_rx_);
  return ((u16_rxBuf_len_ - (uint16_t)_dma_ch->NDTR)) & (u16_rxBuf_len_ - 1);
}
...
void UART_DMAC::set_txbytes(uint8_t *_arr, uint16_t _size) {
  ...
  // DMAを無効化
  LL_DMA_DisableStream((DMA_TypeDef *)dma_, (uint32_t)dma_ch_tx_);
  ...
  // DMAの有効化
  LL_DMA_EnableStream((DMA_TypeDef *)dma_, (uint32_t)dma_ch_tx_);
